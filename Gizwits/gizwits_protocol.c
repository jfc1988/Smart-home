...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagjidianqi)
    {
        info->event[info->num] = EVENT_jidianqi;
        info->num++;
        dataPoints->valuejidianqi = gizStandardDecompressionValue(jidianqi_BYTEOFFSET,jidianqi_BITOFFSET,jidianqi_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagjidianqi2)
    {
        info->event[info->num] = EVENT_jidianqi2;
        info->num++;
        dataPoints->valuejidianqi2 = gizStandardDecompressionValue(jidianqi2_BYTEOFFSET,jidianqi2_BITOFFSET,jidianqi2_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagfengshan)
    {
        info->event[info->num] = EVENT_fengshan;
        info->num++;
        dataPoints->valuefengshan = gizStandardDecompressionValue(fengshan_BYTEOFFSET,fengshan_BITOFFSET,fengshan_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagchuanglian)
    {
        info->event[info->num] = EVENT_chuanglian;
        info->num++;
        dataPoints->valuechuanglian = gizStandardDecompressionValue(chuanglian_BYTEOFFSET,chuanglian_BITOFFSET,chuanglian_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuejidianqi != cur->valuejidianqi)
    {
        GIZWITS_LOG("valuejidianqi Changed\n");
        ret = 1;
    }
    if(last->valuejidianqi2 != cur->valuejidianqi2)
    {
        GIZWITS_LOG("valuejidianqi2 Changed\n");
        ret = 1;
    }
    if(last->valuefengshan != cur->valuefengshan)
    {
        GIZWITS_LOG("valuefengshan Changed\n");
        ret = 1;
    }
    if(last->valuechuanglian != cur->valuechuanglian)
    {
        GIZWITS_LOG("valuechuanglian Changed\n");
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(jidianqi_BYTEOFFSET,jidianqi_BITOFFSET,jidianqi_LEN,(uint8_t *)devStatusPtr,dataPoints->valuejidianqi);
    gizStandardCompressValue(jidianqi2_BYTEOFFSET,jidianqi2_BITOFFSET,jidianqi2_LEN,(uint8_t *)devStatusPtr,dataPoints->valuejidianqi2);
    gizStandardCompressValue(fengshan_BYTEOFFSET,fengshan_BITOFFSET,fengshan_LEN,(uint8_t *)devStatusPtr,dataPoints->valuefengshan);
    gizStandardCompressValue(chuanglian_BYTEOFFSET,chuanglian_BITOFFSET,chuanglian_LEN,(uint8_t *)devStatusPtr,dataPoints->valuechuanglian);
